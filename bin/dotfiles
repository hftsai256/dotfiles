...
# Join array elements for apt command to avoid thread overhead
...
if [[ ! "$(type -P git)" ]] && is_ubuntu; then
  ...
  echo $SUDOPW | sudo -S apt-get -qq install git-core
fi
...
if [[ ! -d $DOTFILES ]]; then
  ...
  git clone --recursive git://github.com/hftsai256/dotfiles.git $DOTFILES
  cd $DOTFILES
elif [[ "$1" != "restart" ]]; then
...
# Export Python 3 virtual environment name
export PYVENV="opt"
